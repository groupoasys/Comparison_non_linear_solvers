...
#      for line in range(0,self.n_lin): #iteration over the lines
 ...
 #     data_filter.to_csv(output_file,index=False)        
  ...
  def learn_line(self,
    ...
    num_trust = 0
    num_right = 0
    # Method 0: Benchmark, it does not learn anything.
    if method == 'bau':
      for line in range(self.y_train.shape[1]):
        score.append(0)
        mat_pred.append([0 for t in range(self.y_test.shape[0])])
        mat_prob.append([0 for t in range(self.y_test.shape[0])])
    # Method 1: KNN
    if method[0:3]=='knn':
      neig = int(method[3:])  
      for line in range(self.y_train.shape[1]):
        y_train_line = self.y_train.iloc[:,line]
        y_test_line = self.y_test.iloc[:,line]      
        if self.weights.count(self.weights[0]) == len(self.weights):
          clf = neighbors.KNeighborsClassifier(neig,weights='distance')
        else:
          clf = neighbors.KNeighborsClassifier(neig,weights='distance',metric='wminkowski',p=2,metric_params={'w': np.abs(np.array(self.weights[line]))})
        clf.fit(self.x_train,y_train_line)
        score.append(clf.score(self.x_test, y_test_line, sample_weight=None))
        mat_pred.append(clf.predict(self.x_test).tolist())
        mat_prob.append(np.max(clf.predict_proba(self.x_test),axis=1).tolist())
    # Method 2: SVM (Salva, needs revision)
    elif method=='svm':
      for line in range(self.y_train.shape[1]):
        ...
        try:
          ...
          prob = np.max(clf.predict_proba(self.x_test),axis=1).tolist()
        except ValueError:
          pred = [y_train_line[0] for i in range(24)]
          prob = [1 for i in range(24)]
          score.append(sum(self.y_test==y_train_line[0])/len(self.y_test)) 
    # Method 3: Tree (Salva, needs revision) 
    elif method=='tree':
      for line in range(self.y_train.shape[1]):
        y_train_line = self.y_train.iloc[:,line]
        y_test_line = self.y_test.iloc[:,line]  
        clf = tree.DecisionTreeClassifier(min_samples_leaf=10)
        clf = clf.fit(self.x_train,y_train_line)
        score.append(clf.score(self.x_test, y_test_line, sample_weight=None))
        pred = clf.predict(self.x_test).tolist()
        prob = np.max(clf.predict_proba(self.x_test),axis=1).tolist()  
    #Method 4: SVM (Asun)
      ...
      SVM_regularization_parameter_grid = [10**range_element for range_element in range(-3, 3)]
    ...
    # Method 5:  learning function of Line Roald
    elif method[0:5]=='roald':
      alpha = int(method[5:])/100
      for l in range(self.n_lin):
        status_min = self.max_flow_roald(ini_train=self.ini_train,end_train=self.end_train,sense=1,line=l+1,alpha=alpha)
        status_max = self.max_flow_roald(ini_train=self.ini_train,end_train=self.end_train,sense=-1,line=l+1,alpha=alpha)
        if status_min and status_max:
          score.append(0)
          mat_pred.append([0 for t in range(self.y_test.shape[0])])
          mat_prob.append([2 for t in range(self.y_test.shape[0])])
        else:
          score.append(0)
          mat_pred.append([0 for t in range(self.y_test.shape[0])])
          mat_prob.append([-1 for t in range(self.y_test.shape[0])])
  
    # Method 6: learning function naive
    elif method=='naive':
      saturation_train = np.round(100*np.mean(abs(self.data.ix[24*self.ini_train:24*self.end_train,'l1':'l120'].values),axis=0),1).tolist()
      for l in range(self.n_lin):
        if saturation_train[l]==0:
          score.append(0)  
          mat_pred.append([0 for t in range(self.y_test.shape[0])])
          mat_prob.append([2 for t in range(self.y_test.shape[0])])
        else:
    ...
    #Method 7: single node. It just ignores all flow constraints.
    elif method=='single':
      for l in range(self.n_lin):
        score.append(0)
        mat_pred.append([0 for t in range(self.y_test.shape[0])])
    ...
    # Method 8: KNN plus juanmi's filter
    if method[0:3]=='knj':
      neig = int(method[3:])  
      data_filtered = pd.read_csv(self.data_file[:-4]+'_filtered2'+'.csv', index_col=False)
      for line in range(self.y_train.shape[1]):
        print('line'+str(line))
        y_train_line = self.y_train.iloc[:,line]
        y_test_line = self.y_test.iloc[:,line]      
        y_train_line_filtered = data_filtered.iloc[24*self.ini_train:24*self.end_train,2*self.n_bus+line]
        y_test_line_filtered = data_filtered.iloc[24*self.ini_test:24*self.end_test,2*self.n_bus+line]
        filtered = sum(y_train_line_filtered==3)
        non_filtered = sum(y_train_line_filtered!=3)
        if self.weights.count(self.weights[0]) == len(self.weights):
          clf = neighbors.KNeighborsClassifier(neig,weights='distance')
        else:
          clf = neighbors.KNeighborsClassifier(neig,weights='distance',metric='wminkowski',p=2,metric_params={'w': np.abs(np.array(self.weights[line]))})
        if filtered == len(y_train_line):
          score.append(1)
          pred = [0 for i in range(24*(self.end_test - self.ini_test))]
          prob = [-1 for i in range(24*(self.end_test - self.ini_test))]
        elif non_filtered < neig:
          score.append(1)
          pred = [0 for i in range(24*(self.end_test - self.ini_test))]
          prob = [-1 for i in range(24*(self.end_test - self.ini_test))]
        else:   
          clf.fit(self.x_train[y_train_line_filtered!=3],y_train_line[y_train_line_filtered!=3])
          score.append(clf.score(self.x_test, y_test_line, sample_weight=None))
          pred = clf.predict(self.x_test).tolist()
          prob = np.max(clf.predict_proba(self.x_test),axis=1).tolist()
        for i,j in enumerate(y_test_line_filtered):
          if j==3:
            pred[i] = 0
            prob[i] = 2 
        mat_pred.append(pred)
    ...
    #Method 9: Perfect
      ...
      for l in range(self.n_lin):
        score.append(0)
        mat_pred.append(self.y_test.iloc[:,l].tolist())
        mat_prob.append([2 for t in range(self.y_test.shape[0])])
    
    # Method 10:  learning function of Zhai
    elif method=='zhai':
      dem = self.data.iloc[24*self.ini_test:24*self.end_test,:self.n_bus].reset_index(drop=True)
      win = self.data.iloc[24*self.ini_test:24*self.end_test,self.n_bus:2*self.n_bus].reset_index(drop=True)
      m = self.max_flow_zhai()
      opt = pe.SolverFactory('cplex',symbolic_solver_labels=True,tee=True)
      opt.options['threads'] = 1   
      for l in range(self.n_lin):
        row_pred = []
        row_prob = []
        for ll in range(self.n_lin):
          m.line[ll+1].value = 0
        m.line[l+1].value = 1
        for t in range(24*(self.end_test-self.ini_test)):
          for b in range(self.n_bus):
            m.dem[b+1].value = dem.iloc[t,b]
            m.win[b+1].value = win.iloc[t,b] 
          m.sense.value = 1
          res_min = opt.solve(m)   
          flow_min = m.z.value
          m.sense.value = -1
          res_max = opt.solve(m)   
          flow_max = m.z.value
          #print('period',t,'fmin',flow_min,'fmax',flow_max,'pmax',self.lin['Pmax'][l])
          if max(abs(flow_min),abs(flow_max)) >= self.lin['Pmax'][l]:
            row_pred.append(0)
            row_prob.append(-1)
          else:
            row_pred.append(0)
            row_prob.append(2)
        mat_pred.append(row_pred)
        mat_prob.append(row_prob)
    
    #Method 11: illustrative m2svm optimization
    elif method == 'illustrative_m2svm_optimization':
        if net_demand:
    ...
    # Method 11:  illustrative example m2svm
    elif method == 'illustrative_m2svm':
        if net_demand:
            x_train = pd.DataFrame(self.data.iloc[self.ini_train:self.end_train,:self.n_bus].values-self.data.iloc[self.ini_train:self.end_train,self.n_bus:2*self.n_bus].values)  
            x_test = pd.DataFrame(self.data.iloc[self.ini_test:self.end_test,:self.n_bus].values-self.data.iloc[self.ini_test:self.end_test,self.n_bus:2*self.n_bus].values)  
        ...
        samples = output_sampling['indexes_individuals_all_samples_and_lines']
        correspondence_time_period_line_all_samples = output_sampling['correspondence_time_period_line_all_samples']
        number_of_individuals_training = len(x_train)
        #Modification of the training_1 sample in order to get independent training sets in all the lines.
        for line in range(number_of_lines):
            samples[line]['training_1'] = list(range(1, number_of_individuals_training))
#        validation_sample = samples[0]['validation']
#        if(len(validation_sample)<number_of_lines):
#            raise error.my_custom_error("Please verify the number of elements in the validation sample")
#        else:
#            number_of_elements_per_line = len(validation_sample)//number_of_lines
#            number_of_extra_elements = len(validation_sample)%number_of_lines
#            for line in range(number_of_lines):
#                samples[line]['training_1'] = sorted(samples[line]['training_1'] + validation_sample[line*number_of_elements_per_line:(line+1)*number_of_elements_per_line])
#                correspondence_time_period_line_all_samples['validation']['line'][correspondence_time_period_line_all_samples['validation']['time_period'].isin(validation_sample[line*number_of_elements_per_line:(line+1)*number_of_elements_per_line])] = line + 1
#            while(number_of_extra_elements > 0):
#                sorted(samples[line]['training_1'].append(validation_sample[number_of_lines*number_of_elements_per_line + number_of_extra_elements].values))
#                line = line + 1
#                number_of_extra_elements = number_of_extra_elements - 1
        correspondence_time_period_line_all_samples['training_1'] = pd.concat([correspondence_time_period_line_all_samples['training_1'],
                                                                              correspondence_time_period_line_all_samples['training_2'],
                                                                              correspondence_time_period_line_all_samples['validation']]).sort_values(by = 'time_period')
        correspondence_time_period_line_all_samples['training_1'].index = range(1, len(correspondence_time_period_line_all_samples['training_1']) + 1)
        ...
        if net_demand:
            ...
            number_of_renewable_energy = 1
        beggining_file_name_to_save_results = 'illustrative_results_by_line_'
        folder_results_msvm = 'results_msvm/'
        if not (os.path.isdir('./' + folder_results_msvm)):
            os.mkdir(folder_results_msvm)
        csv_file_name = 'illustrative_results_all_lines'
        ...
        csv_file = folder_results_msvm + csv_file_name +'_' + approach + '.csv'
        file_to_write = open(csv_file, 'a')
        file_to_write.write('data_file' + ',' + 'line' + ','+ 'approach' + ',' +"%zeros" +',' +"%ones" +',' +"%minus_ones" +',' + '% total accuracy' + ',' + 'SVM reg param' + ',' +'weights'+'\n')
        file_to_write.close()
        ...
        for line in range(1, 2):
            ...
            for SVM_regularization_parameter in SVM_regularization_parameter_grid:
                ...
                sample_by_line = samples[line]
                label_all_samples = ptg.get_label_all_samples(sample_names = sample_names,
                                                      label = label,
                                                      line = line,
                                                      sample_by_line = sample_by_line)
                transformed_label_all_samples = ptg.get_transformed_label_all_samples(label_all_samples = label_all_samples,
                                                                                  label_values = label_values,
                                                                                  new_label_values = new_label_values)
                data_all_samples = ptg.get_data_all_samples(sample_names = sample_names,
                                                            data = data,
                                                            sample_by_line = sample_by_line)
                
                correspondence_time_period_line_all_samples['training_1'] = pd.DataFrame(index = range(1, number_of_individuals_training),
                                                                                         columns = ['time_period', 'line'])
                correspondence_time_period_line_all_samples['training_1']['time_period'] = list(range(1, number_of_individuals_training))
                correspondence_time_period_line_all_samples['training_1']['line'] = [line + 1]*(number_of_individuals_training - 1)
                
                
                kernel_matrix = prediction.get_kernel_matrix(data_all_samples = data_all_samples,
                                                             weights = pd.DataFrame(data = self.weights[line]),
                                                               correspondence_time_period_line_all_samples = correspondence_time_period_line_all_samples,
                                                               sample_by_line = sample_by_line,
                                                               first_sample = sample_names[0],
                                                               second_sample = sample_names[0],
                                                               line = line,
                                                               number_of_renewable_energy = number_of_renewable_energy,
                                                               label_first_sample = label_all_samples['training_1'],
                                                               label_second_sample = label_all_samples['training_1'])
                
                kernel_matrix_test = prediction.get_kernel_matrix(data_all_samples = data_all_samples,
                                                             weights = pd.DataFrame(data = self.weights[line]),
                                                               correspondence_time_period_line_all_samples = correspondence_time_period_line_all_samples,
                                                               sample_by_line = sample_by_line,
                                                               first_sample = sample_names[3],
                                                               second_sample = sample_names[0],
                                                               line = line,
                                                               number_of_renewable_energy = number_of_renewable_energy,
                                                               label_first_sample = label_all_samples['test'],
                                                               label_second_sample = label_all_samples['training_1'])
                
                
                output_first_step[line] = fsap.run_first_step_alternating_approach(SVM_regularization_parameter = SVM_regularization_parameter,
                                                                                     number_of_nodes = number_of_nodes,
                                                                                     ...
                                                                                     correspondence_time_period_line_all_samples = correspondence_time_period_line_all_samples,
                                                                                     sample_by_line = sample_by_line,
                                                                                     initial_variables = self.weights[line],
                                                                                     sample_names = sample_names,
                                                                                     line = line,
                                                                                     label_values = label_values)
                
                
                covariance_matrix = np.cov(np.array(data_all_samples['training_1']).T)
                print(covariance_matrix)
                squared_differences_matrix = {}
                mean_squared_differences = {}
                for dimension in range(1,3):
                    squared_differences_matrix[dimension] = np.einsum('i,i->i', 
                                                                data_all_samples['training_1'][dimension],
                                                                data_all_samples['training_1'][dimension])[:,None] + np.einsum('i,i->i',
                                                                                                                              data_all_samples['training_1'][dimension], 
                                                                                                                              data_all_samples['training_1'][dimension]) - 2*np.einsum('i,j->ij',
                                                                                                                                                                                      data_all_samples['training_1'][dimension], 
                                                                                                                                                                                      data_all_samples['training_1'][dimension].T)
                    mean_squared_differences[dimension] = np.mean(squared_differences_matrix[dimension])
                
                    print("Matrix differences dimension %d =" % dimension)
                    print(squared_differences_matrix[dimension])
                    print("Average matrix differences dimension %d = %f" %( dimension, mean_squared_differences[dimension]))
                
                if (line == 1):
                    pltu.plot_colors_classification(self,
                                           number_of_point_per_axis = 201,
                                            output_first_step = output_first_step,
                                            line = line,
                                            transformed_label_all_samples = transformed_label_all_samples,
                                            data_all_samples = data_all_samples,
                                            sample_by_line = sample_by_line,
                                            ...
                                            results_SVM = output_first_step[line])
                
                
               
                file_name_to_save_results = folder_results_msvm + beggining_file_name_to_save_results + str(line + 1) +'_' + approach+'.pydata'          
                file_to_save = open(file_name_to_save_results, 'wb')
                pickle.dump(output_first_step[line], file_to_save)
                file_to_save.close()
                score.append(output_first_step[line]['accuracy']['test'])
                #mat_pred.append((output_first_step[line]['transformed_predictions']['test']).tolist())
                #mat_prob.append((-0.5*np.ones(shape = len(self.y_test),dtype = float)).tolist())
              
                file_to_read = open(file_name_to_save_results, 'rb')
                results_to_save = pickle.load(file_to_read)
              
                file_to_write = open(csv_file, 'a')
                file_to_write.write(self.data_file + ',' + str((line + 1)) + ','+approach +','+ str(percentage_individuals_of_label_zero)  + ',' +str(percentage_individuals_of_label_one) + ','+str(percentage_individuals_of_label_minus_one)+',' + str(results_to_save['accuracy']['test']) + ',' +str(SVM_regularization_parameter) +',' +','.join(map(str, list(self.weights[line])))+'\n')
                file_to_write.close()
    
    file_name_all_results = folder_results_msvm + 'illustrative_all_results'+ '_' + approach + '.pydata'
    file_to_save = open(file_name_all_results, 'wb')
    pickle.dump((output_first_step, score, mat_pred, mat_prob), file_to_save)
    file_to_save.close()
